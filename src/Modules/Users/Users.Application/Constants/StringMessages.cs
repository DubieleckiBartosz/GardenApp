namespace Users.Application.Constants;

internal static class StringMessages
{
    //Ok
    internal const string EmailSent = "The email has been sent, please check your email.";

    //Errors

    internal const string UnableNewAccount = "Unable to create a new account.";
    internal const string TemplateNotFound = "Template not found.";
    internal const string UserNotFound = "User not found.";
    internal const string RefreshTokenNotFound = "Refresh token not found.";
    internal const string OperationFailed = "Operation failed.";
    internal const string TokenNotActive = "Token is not active.";
    internal const string Unauthorized = "Unauthorized.";
    internal const string UserBlocked = "User is blocked.";
    internal const string PasswordInvalid = "Password invalid.";
    internal const string TokenIsNullOrEmpty = "Token is null or empty.";
}